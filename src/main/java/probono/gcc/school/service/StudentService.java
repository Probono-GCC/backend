...
public class StudentService {
  ...
  public List<StudentResponseDTO> findAllStudents() {
    try {
      List<Users> studentList = studentRepository.findAll();
  ...
  public String updateStudent(String loginId, StudentUpdateRequestDTO requestDto) {
    try {
      ...
      boolean isFirstUpdate = student.getBirth() == null && student.getSex() == null && student.getPwAnswer() == null;

      if (isFirstUpdate) {
        // Ensure birth, sex, and pwAnswer are provided on the first update
        if (requestDto.getBirth() == null || requestDto.getSex() == null || requestDto.getPwAnswer() == null) {
          logger.info("birth or sex or pwAnswer field is null");
          throw new CustomException("Birth, sex, and password answer are required for the first update.", HttpStatus.BAD_REQUEST);
        }
        ...
        throw new CustomException("Password answer can only be set during the first update.", HttpStatus.BAD_REQUEST);
      }
      ...
      logger.error("Unexpected error occurred while updating the student: {}", e.getMessage());
